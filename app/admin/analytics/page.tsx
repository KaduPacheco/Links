      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-sm backdrop-blur">
          ...
          <div className="mt-8 max-w-2xl">
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight text-slate-950">Performance dos links</h1>
          ...
          <CardContent>
            <div className="space-y-3">
              {rankedLinks.map((item, index) => (
                <article key={item.id} className="grid gap-4 rounded-2xl border bg-white p-4 sm:grid-cols-[auto_1fr_auto]">
                  ...
                  <div className="text-left sm:text-right">
                    <p className="text-2xl font-black text-blue-700">{item.click_count}</p>
                    <p className="text-xs font-semibold text-slate-500">Ultimo: {formatDateTime(item.last_clicked_at)}</p>
                  </div>
                </article>
              ))}
            </div>
