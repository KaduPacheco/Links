...
export default async function HomePage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-4 py-6 sm:px-6 lg:px-8">
      <div className="absolute left-1/2 top-0 -z-10 h-96 w-96 -translate-x-1/2 rounded-full bg-cyan-300/25 blur-3xl" />
      <div className="mx-auto grid min-h-[calc(100vh-3rem)] w-full max-w-6xl items-center gap-8 lg:grid-cols-[1fr_440px]">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={String(label)} className="glass-panel rounded-2xl p-4">
                <Icon className="mb-3 h-5 w-5 text-blue-600" aria-hidden="true" />
                <p className="text-sm font-bold text-slate-800">{String(label)}</p>
              </div>
            ))}
          </div>
        ...
        <section className="glass-panel animate-fade-up rounded-[2rem] p-4 shadow-glow sm:p-6">
          <div className="rounded-[1.5rem] border border-blue-100 bg-gradient-to-br from-white to-blue-50/80 p-5">
            ...
            <div className="mb-6 text-center">
              <h2 className="text-2xl font-black tracking-tight text-slate-950">Links oficiais</h2>
            ...
            <div className="space-y-5">
              {Object.entries(groupedLinks).map(([category, categoryLinks]) => (
                <div key={category} className="space-y-3">
                  <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-700">{category}</p>
                  {categoryLinks.map((item) => (
                    <a
                      key={item.id}
                      href={`/api/click?linkId=${item.id}`}
                      className="group flex items-center gap-4 rounded-2xl border border-white bg-white p-4 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-900/10 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-blue-700 transition group-hover:bg-blue-600 group-hover:text-white">
